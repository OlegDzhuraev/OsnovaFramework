...
namespace OsnovaFramework
{
    public sealed class Filter<X> : IEnumerable<X> where X : BaseComponent
    {
        ...
        public X FirstWithout<T>() where T : BaseComponent => 
            results.FirstOrDefault(x => x && !x.Entity.Has<T>());
        
        public Filter<T> GetAllWith<T>() where T : BaseComponent =>
            new (results.Select(x => x as T));
        
        public Filter<X> With<T>() where T : BaseComponent => 
            new (results.Where(x => x.Entity.Has<T>()));

        public Filter<X> Without<T>() where T : BaseComponent => 
            new (results.Where(x => !x.Entity.Has<T>()));
